...
if len(sys.argv) != 3:
    print ('Usage: python3 ./TimetableCNF.py <input.py> <output.txt>', file = sys.stderr)
    sys.exit(1)

input_data = sys.argv[1]
output = sys.argv[2]

input_spec = util.spec_from_file_location('', input_data)
input_module = util.module_from_spec(input_spec)
input_spec.loader.exec_module(input_module)
for obj in dir(input_module):
    globals()[obj] = input_module.__dict__[obj]
...
if (W_num == 2):
    ...
    print_res_len (CNF_dict, 7)
print ('The final CNF in DIMACS format is in the file', output)
print()
