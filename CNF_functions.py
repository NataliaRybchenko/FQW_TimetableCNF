...
def add_dis_one_pol (pol, CNF_dict, keys_neg, X_neg, in_dis_num, X_possible_str, X_possible_fs):
    forbiddenDis = set()
    Cnk = combinations(X_neg, in_dis_num)
    for Cnk_i in Cnk:
        ...
        for s in keys_neg:
            if sign:
                if in_dis_num > s:
                    Cnk_i_others = list(set(X_neg) - Cnk_i)
...
def make_part_one_pol (pol, CNF_dict, keys, X7, num):
    DIS_part = set()
    forbiddenDis = set()
    for s in keys:
        if num >= s:
            Cnk = combinations(X7, s)
            for Cnk_i in Cnk:
                Cnk_i = set(Cnk_i)
                Cnk_i_pol = set()
                if pol == '-':
                ...
                if Cnk_i_pol in CNF_dict[pol][s]:
                    Cnk_i_others = list(set(X7) - Cnk_i)
